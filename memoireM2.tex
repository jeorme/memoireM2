...
\begin{itemize}
\item[i)] identification du modèle
\item[ii)] estimation des paramètres
\item[iii)] vérification
\end{itemize}
...
\begin{figure}[h]
	...
	\end{center}
\end{figure}
\captionof{figure}{RNN}
Dans le diagramme ci dessus $A$ correspond à un réseau de neurones qui prend en entrée la variable $x_t$ et qui donne en sortie $h_t$. La boucle signifie que l'information de l'état précédent est passé à l'état actuel. Afin de comprendre comment l'information est passé d'un état à l'autre, on peut représenter de manière séquentielle les réseaux de neurones récurrents~: 
...
\begin{figure}[h]
	...
	\end{center}
\end{figure}
\captionof{figure}{RNN : représentation séquentielle}

Les RNNs les plus simples se décrivent de la manière suivante~:
\begin{Def}\label{VanillaRNN}
Soient $n,p,k\in \N$, $x_0,...,x_m$ une suite d'entrée, $W\in \R^{k,n}$, $W_h\in \R^{k,k}$, $O\in \R^{p,k}$ et $h_{-1}\in\R^k$ alors la dynamique du RNN Vanille (ou Vanilla RNN) associée est décrite par~:
\begin{eqnarray}
h_t &=& f(Wx_t+W_gh_{t-1})\\
y_t&=&Oh_t
\end{eqnarray}
avec $h_t$ l'état de la couche cachée, $y_t$ l'état de la couche de sortie et $f$ une fonction différentiable. Dans la suite on utilisera indifféremment RNN pour \textit{RNN Vanille}.
\end{Def}



Dans le cas où la fonction d'activation est la fonction $\tanh$, on a~: 
\newline
...
\begin{figure}[h]
	...
	\end{center}
	\captionof{figure}{RNN Vanille avec fonction d'activation $\tanh$}
\end{figure}




On peut également visualiser ces RNNs sous forme de graphe~:
\begin{center}
\def\layersep{2.5cm}
...
\end{tikzpicture}
\end{center}
Les RNNs forment un modèle très puissant, ils sont Turing complet et on peut implémenter n'importe quel algorithme avec cette architecture. En théorie les RNN sont capables de prédire les données futures dans le cas de dépendance à long terme, mais en pratique ce n'est pas le cas. Cela vient du problème du \textit{vansihing or exploding gradient}. Lors de la descente du gradient temporelle BTT (\textit{Backpropagation Through Time}) le gradient s'annule ou explose numériquement \cite{Bengio}. C'est pour remédier à ce problème du \textit{vanishing gradient} que le modèle LSTM a été créé \cite{LSTM}
\subsection{LSTM} 
L'architecture LSTM a permis de résoudre le problème du \textit{vansishing gradient} et de permettre l'apprentissage lors de dépendance à long terme. Le principe des réseaux LSTM reposent sur l'utilisation de cellules d'états (\textit{cell state}) qui permettent de transporter l'information de cellule en cellule et de portes (\textit{gates}). Les portes permettent de filtrer l'information et de contrôler ce que garde comme l'information la cellule d'état. Dans le schema ci-dessous la cellule d'état est l'information qui traverse la cellule en haut. L'entrée est $C_{t-1}$ auquel on va multipliée une valeur puis ajouter une valeur est en utilisée la valeur de la cellule d'état $C_t$ pour calculer la sortie du cellule.

 \begin{figure}[h]
 \begin{center}
   \includegraphics[scale=.7]{LSTM.png}
\captionof{figure}{architecture d'un LSTM}
\end{center}
\end{figure}


La fonction $\sigma$ définie dans l'architecture du LSTM correspond à la fonction sigmoïde. Les symboles multiplcations (resp. additions) correspondent à la multiplication (resp. addition) point par point. Les équation de la dynamique de ce modèle sont les suivantes~:
\begin{eqnarray}
f_t&=&\sigma(U^fx_t+W^fh_{t-1}+b_f)\\
i_t&=&\sigma(U^ix_t+W^ih_{t-1}+b_i)\\
o_t&=&\sigma(U^ox_t+W^oh_{t-1})\\
C_t&=&C_{t-1}f_t+\tanh\left(U^{\tilde{c}}x_t+W^{\tilde{c}}h_{t-1}\right)i_t\\
y_t&=&O\tanh\left(c_t\right)o
\end{eqnarray}
Le terme $f_t$ va être multiplié à la cellule $C_{t-1}$, cette opération est réalisée par la porte d'oubli (\textit{forget gate}). Elle permet de pondérer l'état précédent en effet la fonction sigmoïde retourne des valeurs comprises entre 0 et 1 et donc si $f_t$ vaut zero cela signifie que l'on ne tient pas compte de l'état précédent.

Après ce passage on va modifier la cellule d'état $C_{t-1}$ afin d'obtenir la valeur à l'instant $t$. Cette opération est réalisée par l'\textit{input gate}. Cette porte va ajouter une information à la cellule $C_{t-1}$ afin d'obtenir la cellule $C_t$. L'\textit{input gate} réalise l'opération suivante~:
\begin{align*}
C_t&=\sigma(U^fx_t+W^fh_{t-1}+b_f) *C_{t-1}+\sigma(U^ix_t+W^ih_{t-1}+b_i)* \tanh\left(U^{\tilde{c}}x_t+W^{\tilde{c}}h_{t-1}\right)\\
&=f_t*C_{t-1}+i_t*\tilde{C}_t
\end{align*}

Après le passage de la deuxième porte on a mis à jour la cellule d'état $C_t$ en fonction des données passées et des nouvelles données. On utilise alors cette cellule d'état pour déterminer la valeur de sortie. Cette opération est réalisée par la porte de sortie (\textit{output gate}). Afin de déterminer quelle valeur on va sortir, on va filtrer la valeur $C_t$ à l'aide de la porte de sortie. Pour cela on applique une fonction sigmoide sur les données d'entrée que l'on va multiplier à $\tanh(C_t)$. On utilise $\tanh$ afin de normalisée la sortie sur l'intervalle $[-1,1]$, la valeur de sortie $h_t$ est~:
\begin{align*}
h_t&=\sigma(U^ox_t+W^oh_{t-1})\tanh\left(C_t\right)\\
&=o_t\tanh\left(C_t\right)
\end{align*}
\subsection{GRU}
En 2014 \cite{GRU} ont définit une version simplifiée du LSTM, appelée GRU (\textit{Gates Recurrent Unit}). Ce modèle possède moins de paramètres que le modèle LSTM~:

 \begin{figure}[h]
 \begin{center}
   \includegraphics[scale=.7]{LSTM3-var-GRU.png}
\captionof{figure}{architecture d'un LSTM}
\end{center}
\end{figure}
Parmi les différences notables, on remarque que le réseau GRU combine les portes d'entrée et d'oubli en une seule porte appelée pourt de mise à jour (\textit{update gate}) et qu'il combine la cellule d'état $C_t$ avec la cellule $h_t$.


Dans \cite{PerfLstmGru}, les auteurs démontrent que le modèle GRU a des performances équivalentes au modèles LSTM de base.
\section{Annexe}
...
Pour un processus définit à partir de bruit blanc, on aimerait pouvoir exprimer ce processus en fonction du passé (observé) et pas seulement en fonction du bruit blanc non observé. C'est la question de l'inversibilité du processus. Cela veut donc dire qu'un processus est inversible si on peut le représenter comme une auto-régression infinie $AR(\infty)$. Ainsi un processus AR(p) est toujours inversible et pour un processus MA(q) la condition d'inversibilité est similaire à la condition de stationnarité des processus AR~:
\begin{pro}
Un processus MA(q) défini par : $X_t=\Theta(L)\epsilon_t$, avec $\epsilon_t$ un bruit blanc et $\theta_0=1$, est inversible si les racines du polynôme $ \Theta(L)$ sont, en module, strictement supérieures à 1
\end{pro}
\subsection{Densité spectrale}
Les coefficients d'auto-covariance d'une série stationnaire correspondent aux coefficients de Fourier d'une mesure positive, appelée mesure spectrale du processus. Il est possible de montrer que cette mesure spectrale admet un densité, dite spectrale, par rapport à la mesure de Lebesgue sur $[-\pi,\pi]$ que nous noterons $f_X$.
\begin{Def}
Soit $\Xt$ un processus de fonction d'auto-covariance $\gamma_X$ la densité spectrale $f_X$ de $\Xt$ s'écrit~:
$$
...
$$
\end{Def}
Le principe de vraisemblance revient à déterminer la valeur du paramètre $\theta$, fonction des observations $x_1,...,x_n$, qui assure la plus grande probabilité d'apparition de ces observations $(x_1,...,x_n)$.
\begin{corollary}
...
Lorsque le processus est bien estimé, les résidus entre les valeurs observées et les valeurs estimées par le modèle doivent se comporter comme un bruit blanc. On notera par la suite $\hat{\epsilon}_t$ le résidu d'estimation du modèle. Pour un modèle ARMA ou ARIMA on suppose que le processus $\epsilon_t$ est un bruit blanc gaussien centré de variance $\sigma^2$, ainsi par le théorème centrale limite on doit avoir :
$$
\frac{\overline{\epsilon}_t\sqrt{T}}{\sigma^2}\rightarrow_{T\rightarrow\infty}\mathcal{N}(0,1)
$$
avec $\overline{\epsilon}_t$ la moyenne des résidus. on peut donc test la nullité de la moyenne des résidus en construisant un intervalle de confiance au seuil adéquat.

On peut aussi vérifier l'auto-corrélation des résidus via les tests de Durbin-Watson, test de Box et Pierce ou de Ljung-Box par exemple. Si les résidus obéissent à un bruit blanc il ne doit pas y avoir d'auto-corrélation dans la série.
...
Pour les modèles AR, MA, ARMA et ARIMA par exemple plutôt que d'utiliser les métriques classiques telles que MAE, RMSE par exemple on va utiliser les critères tenant compte de la complexité du modèle~:
\begin{itemize}
